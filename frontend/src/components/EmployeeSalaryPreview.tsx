...
import { SessionStore } from "@/auth/session"; // Assuming you're using a custom session store
...
const EmployeeSalaryPreview: React.FC<Props> = ({ employee, start, end, onClose }) => {
  const [pdfUrl, setPdfUrl] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  // Assuming you have a way to get the logged-in user
  const loggedInUser = employee;  // In this case, the employee is the logged-in user
  ...
  useEffect(() => {
    const fetchPDF = async () => {
      try {
        ...
        const url = `${process.env.NEXT_PUBLIC_DOMAIN}/generate-pdf/salary-breakdown-emp/?${params}`;
...
export default EmployeeSalaryPreview;
